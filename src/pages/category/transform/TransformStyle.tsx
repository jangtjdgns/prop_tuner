...
const TransformStyle: React.FC = () => {
    // 3d 변환에서 부모-자식 관계를 설정 (perspective 속성 사용 필요)
    // flat - 자식 요소가 3D 변환 효과를 받지 않고 2D 평면에서 렌더링 됨
    // preserve-3d - 자식 요소가 부모의 3D 컨텍스트 내에서 렌더링 됨
            ...
            <div id='option-wrap' className='absolute top-10 left-6 transition-transform duration-500 z-[1000]'>
                <div className='w-72 flex flex-col gap-2 shadow rounded-xl p-2 bg-white border border-gray-200 hover:shadow-2xl hover:border-gray-300 transition-all duration-500'>
                    ...
                    <div className='flex flex-col gap-2'>
                        <div className='text-center pt-2 font-bold text-lg'>Transform Style</div>
                    </div>
                    ...
                    <div id='option-wrap-bottom' className='flex flex-col gap-2 max-h-[360px] overflow-y-scroll px-2'>
                        <div className='text-center p-0.5 text-xs'>
                            ...
                            <button className='copy-css-btn btn btn-square btn-ghost btn-xs ml-2 flip-horizontal-bottom'
                                onClick={() => copyCss('transform-style', transformStyle)}
                            >
                        ...
                        <div className='grid grid-cols-2 items-center text-center gap-2'>
                            <input type='radio' name='backface' className="btn" aria-label={'flat'}
                                checked={transformStyle === 'flat'}
                                onChange={() => setTransformStyle('flat')}
                            />
                            <input type='radio' name='backface' className="btn" aria-label={'preserve-3d'}
                                checked={transformStyle === 'preserve-3d'}
                                onChange={() => setTransformStyle('preserve-3d')}
                            />
                        </div>
